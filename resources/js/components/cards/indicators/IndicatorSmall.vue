<template>
    <div class="flex space-x-2.5 mb-4">
        ...
        <div>
            <div class="text-base text-black mb-2.5">{{ title }}</div>
            <div v-if="!indicatorValue" class="text-black text-2xl font-bold">-</div>

            <div
                v-else
                class="text-black text-2xl font-bold flex flex-row space-x-1 items-center"
            >
                <span v-if="arrow" class="inline-block top-px relative mr-1">
                    <img
                        v-show="arrowType === 'up'"
                        :src="arrowUpUrl"
                        width="24"
